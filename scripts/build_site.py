...
def future_films():
    ...
    for i, film in enumerate(films):
        ...
        panels.append(f'''<article class="future-panel" id="future-panel-{key}" role="tabpanel" aria-labelledby="future-tab-{key}" tabindex="0"{'' if i == 0 else ' hidden'}><div class="film-frame"><video id="film-{key}" class="concept-video" data-src="/assets/video/{key}.mp4" poster="/assets/video/{key}-poster.jpg" width="1280" height="720" muted loop playsinline preload="none" aria-label="{E(film['title'], quote=True)}を描くコンセプト映像" aria-describedby="future-film-caption"></video><button class="film-control" data-film-toggle="film-{key}" aria-controls="film-{key}" aria-label="映像を再生" aria-pressed="false"><span data-film-icon aria-hidden="true">▶</span><span data-film-label>再生</span></button><p class="film-error" hidden>映像を読み込めませんでした。<button type="button" data-film-retry="film-{key}">再読み込み</button></p></div><div class="future-panel-copy"><p class="eyebrow">{E(film['label'])}</p><h3>{E(film['title'])}</h3><p>{E(film['copy'])}</p></div></article>''')
...
def service_card(s):
    ...
    return f'<article class="service-card"><p class="eyebrow">{s["label"]}</p><h2>{title}</h2><p>{E(s["description"])}</p><a class="text-link" href="{s["path"]}/">詳しく見る <span aria-hidden="true">↗</span></a></article>'
...
if __name__ == '__main__':
    ...
    names = {
        ...
        '/service/dtf': ('Dealer Transformation Fund', 'HANDS-ON MANAGEMENT / 準備中', 'ディーラーの、その先をともに。', None),
        '/service/x-iuiE4-': ('Lymo Local Partner', 'LOCAL MOBILITY / BPO', '地域に、新しい移動の選択肢を。', None)
    }
    for s in services:
    ...
    corporate_cards = '''<article class="service-card"><p class="eyebrow">01 / FIELD INTELLIGENCE</p><h2>Dealer AX</h2><p>業務設計、AI活用、実装・運用を一体で支援。現場の変革を、お客様と向き合う時間へつなげます。</p><a class="text-link" href="/service/lymo-series/#dealer-ax">詳しく見る <span aria-hidden="true">↗</span></a></article><article class="service-card"><p class="eyebrow">02 / OPERATING SYSTEM</p><h2>Dealer OS</h2><p>顧客・車両・店舗と業務ルールを共通のモデルへ。現場の理解、判断、実行を支える基盤を構築します。</p><a class="text-link" href="/service/lymo-series/#dealer-os">構想を見る <span aria-hidden="true">↗</span></a></article>'''
    ...
    for s in services:
        ...
        if s['photo']:
            photo = f'<figure><img src="/assets/photos/{s["photo"]}" alt="' + ('Lymoの撮影より、車のそばで会話する二人' if s['photo'] != 'dealer.webp' else '自動車販売店の外観') + '" loading="lazy"><figcaption>現場から、移動の新しい可能性へ。</figcaption></figure>'
        else:
        ...
        platform_details = ''
        if s['path'] == '/service/lymo-series':
            platform_details = '''<div class="service-platform-grid"><section id="dealer-ax"><p class="eyebrow">01 / FIELD INTELLIGENCE</p><h2>Dealer AX</h2><h3>AIを、現場の力に。</h3><p>日々の業務を知るところから、変革は始まる。業務設計、AI活用、実装・運用を一体で支援し、お客様と向き合う時間を生み出します。</p><div class="tags"><span>業務設計</span><span>AI実装</span><span>運用支援</span></div><a class="text-link" href="/contact/">Dealer AXについて相談する <span aria-hidden="true">↗</span></a></section><section id="dealer-os"><p class="eyebrow">02 / OPERATING SYSTEM</p><h2>Dealer OS</h2><h3>現場のつながりを、共通の基盤へ。</h3><p>顧客、車両、店舗、整備、契約。対象同士の関係、状態、業務ルールを表したOntologyを土台に、現場の理解、判断、実行を支える基盤を構築します。</p><div class="tags"><span>業務モデル</span><span>判断・実行</span><span>共通プロダクト</span></div><a class="text-link" href="/#ontology">業務モデルの構想を見る <span aria-hidden="true">↗</span></a></section></div>'''
